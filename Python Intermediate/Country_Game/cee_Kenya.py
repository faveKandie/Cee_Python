#This is the main program for the cee_Kenya Game
import turtle
import time
import pandas
...
rate = turtle.Screen()
rate.title("cee_Kenya")

doc = pandas.read_csv("Kenya_8States.csv")
listdoc = doc["states"].to_list()

kenyaa_img = "Kenyaa.gif"
rate.addshape(kenyaa_img)
rate.setup(800, 710)
turtle.shape(kenyaa_img)
rate.tracer(100)
...
def func():
    ...
    elif state_text == "Exit" or score == 8:
        pass
...
while len(guessed_state) != 9:
    ...
    if state_text in listdoc and state_text not in guessed_state:
        ...
        scoree.clear()
        scoree.write(f"{score}/8", align = "center", font = ("ComicSans MS", 20, "bold"))
    if  score == 8:
        time.sleep(1)
        rate.clear()
        rate.bgcolor("black")
